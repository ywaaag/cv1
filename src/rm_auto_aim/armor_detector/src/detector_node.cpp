...
{
  auto armors = detectArmors(img_msg);//检测装甲板

  if (pnp_solver_ != nullptr) {
    armors_msg_.header = armor_marker_.header = text_marker_.header = img_msg->header;//设置消息头
    armors_msg_.armors.clear();
    marker_array_.markers.clear();//清空之前的marker
    armor_marker_.id = 0;
    text_marker_.id = 0;

    auto_aim_interfaces::msg::Armor armor_msg;//装甲板消息
...
std::unique_ptr<Detector> ArmorDetectorNode::initDetector()
{
  ...
  int binary_thres = declare_parameter("binary_thres", 160, param_desc);//二值化阈值
//检测颜色
...
//装甲板检测参数
  Detector::ArmorParams a_params = {
    .min_light_ratio = declare_parameter("armor.min_light_ratio", 0.7),//灯条长度最小比例
    .min_small_center_distance = declare_parameter("armor.min_small_center_distance", 0.8),//小装甲板灯条中心最小距离
